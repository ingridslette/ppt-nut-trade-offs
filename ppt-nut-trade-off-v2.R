...
total_biomass_precip <- biomass_precip %>%
  group_by(site_code, site_name, year, year_trt, trt, block, plot, ppt, avg_ppt, sd_ppt) %>%
  summarise(total_biomass = sum(mass), .groups = "drop") %>%
  filter(total_biomass > 0) %>%
  mutate(precip_z = (ppt - avg_ppt) / sd_ppt,
         log_mass = log10(total_biomass))

unique(total_biomass_precip$site_code)
...
# Fit y ~ x for one group and return the slope + its 95% CI + p-value.
# Returns NA fields (rather than erroring) if there isn't enough data or
# there's no variation in x - this happens for e.g. rare species with few
# nonzero observations.
get_slope_stats <- function(data, xvar, yvar, min_n = 3) {
  data <- data[!is.na(data[[xvar]]) & !is.na(data[[yvar]]), ]
  
  if (nrow(data) < min_n || length(unique(data[[xvar]])) < 2) {
...
# Classify a CI as a positive, negative, or "no" response, based on whether
# the interval excludes zero.
classify_response <- function(ci_low, ci_high) {
  case_when(
...
# Variance of a log response ratio (Hedges, Gurevitch & Curtis 1999),
# built from each group's mean, SD, and sample size.
...
### -------------------------------------------------------------------------
### 1. BIOMASS: precipitation-response slopes (control plots only, per site)
### -------------------------------------------------------------------------

site_slopes <- total_biomass_precip %>%
  filter(trt == "Control") %>%
  group_by(site_code) %>%
  group_modify(~ get_slope_stats(.x, "precip_z", "log_mass")) %>%
  ungroup() %>%
  rename(control_slope = slope, control_se = se,
         control_ci_low = ci_low, control_ci_high = ci_high,
         control_p = p_value, control_n = n)

### 2. BIOMASS: NPK log response ratio + its CI, per site

site_trt_mass_stats <- total_biomass_precip %>%
  group_by(site_code, trt) %>%
  summarize(
...
site_lrr_mass <- site_trt_mass_stats %>%
...
site_slopes_lrr_mass <- left_join(site_slopes, site_lrr_mass, by = "site_code") %>%
  mutate(
    ...
    npk_response    = classify_response(lrr_mass_ci_low, lrr_mass_ci_high),
    quadrant = paste(npk_response, precip_response, sep = " / ")
  )

site_slopes_lrr_mass <- site_slopes_lrr_mass |> 
  filter(!is.na(control_slope))

mass_quadrant_counts <- site_slopes_lrr_mass %>%
...
print(mass_quadrant_counts)

mass_quadrant_counts2 <- site_slopes_lrr_mass %>%
  filter(!is.na(precip_response), !is.na(npk_response)) %>%
...
site_mass_fig <- ggplot(site_slopes_lrr_mass,
...
mass_quadrant_bar <- ggplot(mass_quadrant_counts,
...
mass_quadrant_bar2 <- ggplot(mass_quadrant_counts2,
...
site_mass_model2 <- lmer(log_mass ~ trt * precip_z + (1 | site_code/plot) + (1 | site_code:year), 
                         data = total_biomass_precip)
summary(site_mass_model2)



## Create dataframe for biomass functional group analysis
fg_biomass <- biomass6 %>%
  filter(category %in% c("GRAMINOID", "FORB", "WOODY", "LEGUME"))

# Add rows for categories found in some but not all plots and years at a site
years_by_site_biomass <- fg_biomass %>%
  distinct(site_code, year, year_trt)

plots_biomass <- fg_biomass %>%
  distinct(site_code, site_name, block, plot, subplot, trt)

category_by_site <- fg_biomass %>%
  distinct(site_code, category)

fg_biomass_full_design <- plots_biomass %>%
  left_join(category_by_site, by = "site_code") %>%
  left_join(years_by_site_biomass, by = "site_code")

biomass_focal_cols <- fg_biomass %>%
  distinct(site_code, plot, category, year, mass)

fg_biomass_complete <- fg_biomass_full_design %>%
  left_join(biomass_focal_cols,
            by = c("site_code", "plot", "category", "year")) %>%
  mutate(mass = replace_na(mass, 0))

# Join precip data to 0-filled fg biomass data
fg_biomass_precip <- fg_biomass_complete %>%
